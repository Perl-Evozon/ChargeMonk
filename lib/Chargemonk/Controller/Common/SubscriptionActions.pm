package Chargemonk::Controller::Common::SubscriptionActions;
...
Chargemonk::Controller::Common::SubscriptionActions - Catalyst Controller
...
sub cancel_subscription : Local {
	...
	my $link_user_subscription_row =
		$c->model('Chargemonk::LinkUserSubscription')->find( $c->req->param('link_user_subscription_id') );
...
sub upgrade_choose : Local : Args(2) {
	...
	my $link_subscription = $c->model('Chargemonk::LinkUserSubscription')->find($link_subscription_id);

	my @upgrades_to =
		$c->model('Chargemonk::SubscriptionUpgradeTo')
...
sub upgrade : Local : Args(2) {
	...
	my $new_subscription = $c->model('Chargemonk::Subscription')->find( $c->req->param('subscription') );

	my $link_subscription =
		$c->model('Chargemonk::LinkUserSubscription')->find( $link_subscription_id, {prefetch => 'subscription'} );
...
sub upgrade_save : Local : Args(2) {
	...
	try {
		$new_link_user_subscription = $c->model('Chargemonk::LinkUserSubscription')->create(
			{   user_id            => $user_id,
...
sub downgrade_choose : Local : Args(2) {
	...
	my @downgrades_to =
		$c->model('Chargemonk::SubscriptionDowngradeTo')
...
sub renew : Local : Args(2) {
	...
	my $new_subscription = $c->model('Chargemonk::Subscription')->find($subscription_id);
