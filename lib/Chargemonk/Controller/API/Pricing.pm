package Chargemonk::Controller::API::Pricing;
...
sub index : Path(/api/pricing) : Public {
    ...
    my @groups;
    foreach my $group ( $c->model('Chargemonk::SubscriptionGroup')->search( {}, {order_by => 'id'} )->all ) {

        my @subscriptions;
        foreach my $subscription (
            $c->model('Chargemonk::Subscription')->search_rs( {subscription_group_id => $group->id}, {order_by => 'id'} )
            ->all )
        {

            my @features =
                $c->model('Chargemonk::LinkSubscriptionFeature')->search( {subscription_id => $subscription->id} )->all;
