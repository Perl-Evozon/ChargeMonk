package Chargemonk::Controller::Admin::Users;
...
use Chargemonk::Helpers::Admin::UserDetails qw(send_set_user_psw_email);
...
Chargemonk::Controller::Admin::Users - Catalyst Controller
...
sub users : Path : Args(0) {
    ...
    my $total_users  = $c->model('Chargemonk::User')->count();
    ...
    if (%search_criteria) {
        @users = $c->model('Chargemonk::User')->search_rs($where_clause)->all();
    }
    else {
        @users = $c->model('Chargemonk::User')->all();
    }
...
sub user_details : Local : Args(1) {
    ...
    my $user;
    try {
        $user = $c->model('Chargemonk::User')->find( $c->req->args->[0] );
    }
    catch {
...
sub create_user : Local : Args(0) : OnError(add_user) {
    ...
    try {
        my $user = $c->model('Chargemonk::User')->create($user_hash);
...
sub save_user : Local('user_details') : Args(1) : OnError(edit_user) {
    ...
    my $link_user_subscription = $c->model('Chargemonk::LinkUserSubscription')->search( {user_id => $user->id} )->first();

    # establish the user user_type ('ADMIN', 'LEAD' or 'CUSTOMER)
    my $user_type = ( $c->req->param('admin_user') ? 'ADMIN' : 'LEAD' );
    $link_user_subscription =
        $c->model('Chargemonk::LinkUserSubscription')->find( {user_id => $user->id, active => 1} );
