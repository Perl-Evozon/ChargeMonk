package Chargemonk::Controller::Admin::Themes;
...
Chargemonk::Controller::Admin::Themes - Catalyst Controller
...
sub themes : Path : Args(0) {
    ...
    my @themes = $c->model('Chargemonk::Theme')->search( {}, {order_by => {-asc => 'id'}} )->all;
...
sub add_theme : Local : Args(0) {
    ...
    if ( $c->req->param('active') ) {
        $c->model('Chargemonk::Theme')->search()->update( {active => 0} );
    }

    try {
        $c->model('Chargemonk::Theme')->create(
...
sub delete_theme : Local : Args(1) {
    ...
    my $theme = $c->model('Chargemonk::Theme')->find( {id => $c->request->arguments->[0]} );
...
sub activate : Local : Args(0) {
    ...
    $c->model('Chargemonk::Theme')->find( {id => $c->req->param('theme_id')} )->update( {active => 1} );
