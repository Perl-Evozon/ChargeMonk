package Chargemonk::Controller::Admin::Subscriptions;
...
use Chargemonk::Helpers::Common::DateTime qw(convert_to_days);

BEGIN { extends 'Chargemonk::Controller::Authenticated'; }

=head1 NAME

Chargemonk::Controller::Admin::Subscription - Catalyst Controller
...
sub save : Local : Args(0) : OnError(add_subscription) {
    ...
    my $subscription;
    try {
        $subscription = $c->model('Chargemonk::Subscription')->create( $self->_initial_hash($c) );

        #link the created campaings to the new campaign
        $c->model('Chargemonk::LinkCampaignsSubscription')->create(
...
sub edit_subscription : Local : Args(1) {
    ...
    my $reload_data     = $self->_reinitialize_subscription_data($c);
    my $subscription    = $c->model('Chargemonk::Subscription')->find($subscription_id);
...
sub subscription_details : Local : Args(1) {
    ...
    try {
        $group      = $c->model('Chargemonk::SubscriptionGroup')->find( $subscription->subscription_group_id() );
        @upgrades   = $c->model('Chargemonk::SubscriptionUpgradeTo')->search( {subscription_id => $subscription->id()} );
        @downgrades = $c->model('Chargemonk::SubscriptionDowngradeTo')->search( {subscription_id => $subscription->id()} );
        @subscriptions_from_group = $c->model('Chargemonk::Subscription')
            ->search( {subscription_group_id => $subscription->subscription_group_id()} );
        @link_subscriptions_features =
            $c->model('Chargemonk::LinkSubscriptionFeature')->search( {subscription_id => $subscription->id()} );
    }
    catch {
...
sub end : ActionClass('RenderView') {
    ...
    my @subscriptions               = $c->stash->{subscriptions} || $c->model('Chargemonk::Subscription')->all();
...
sub _update_subscription_data : Private {
    ...
    foreach ( $c->model('Chargemonk::LinkSubscriptionFeature')->search( {subscription_id => $subscription->id} )->all ) {
        $_->delete;
    }
    ...
    if ( $c->req->param('import_features_from_subscr') ) {
        ...
        my @imported_features = $c->model('Chargemonk::LinkSubscriptionFeature')
    ...
    foreach my $id (@features) {
        $c->model('Chargemonk::LinkSubscriptionFeature')->update_or_create(
            {   'subscription_id' => $subscription->id,
                'feature_id'      => $id
            }
        );
    ...
    foreach ( $c->model('Chargemonk::SubscriptionUpgradeTo')->search( {subscription_id => $subscription->id} )->all ) {
        $_->delete;
    }
    my @upgrades = $c->req->param('selected_upgrades');
    foreach my $id (@upgrades) {
        $c->model('Chargemonk::SubscriptionUpgradeTo')->update_or_create(
            {   'subscription_id'         => $subscription->id,
    ...
    foreach ( $c->model('Chargemonk::SubscriptionDowngradeTo')->search( {subscription_id => $subscription->id} )->all ) {
        $_->delete;
    }
    my @downgrades = $c->req->param('selected_downgrades');
    foreach my $id (@downgrades) {
        $c->model('Chargemonk::SubscriptionDowngradeTo')->update_or_create(
            {   'subscription_id'           => $subscription->id,
    ...
    my @subscriptions_from_group =
        $c->model('Chargemonk::Subscription')->search( {subscription_group_id => $subscription->subscription_group_id()} );
    ...
    if ( my $campaigns = $c->req->param('added_campaigns') ) {
        ...
        foreach (@camp_ids) {
            next if !$_;
            my $camp = $c->model('Chargemonk::Campaign')->find($_);
