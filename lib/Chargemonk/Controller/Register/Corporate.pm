package Chargemonk::Controller::Register::Corporate;
...
Chargemonk::Controller::Register::Corporate - Catalyst Controller
...
sub step_1 : Path('/register/corporate/step-1') : Args(2) {
    my ( $self, $c ) = @_;
    ...
    $c->stash(
        {   template      => 'register/corporate/step-1.tt',
...
sub step_2 : Path('/register/corporate/step-2') : Args(2) : OnError(step_1) {
    my ( $self, $c ) = @_;
    ...
    $c->stash(
        {   template      => 'register/corporate/step-2.tt',
            required_data => $required_data,
        }
    );
...
sub step_2_confirmation : Path('/register/corporate/step-2-confirmation') : Args(2) {
    my ( $self, $c ) = @_;
    ...
    my $link_subscription = $c->model('Chargemonk::LinkUserSubscription')->create(
        {   user_id            => $user_rs->id,
    ...
    Chargemonk::Helpers::Visitor::Registration::send_register_user_email(
        {   c                         => $c,
            user_id                   => $user_rs->id,
            link_user_subscription_id => $link_subscription->id,
    ...
    $c->stash(
        {   template     => 'register/corporate/step-2-confirmation.tt',
            subscription => $subscription,
            features     => \@features,
            trial        => $c->request->arguments->[0] eq 'trial' ? 1 : 0,
        }
...
sub step_3 : Path('/register/corporate/step-3') : Args(2) {
    my ( $self, $c ) = @_;
    ...
    my $template            = 'register/corporate/step-3-' . $active_gateway . '.tt';
...
sub step_4 : Path('/register/corporate/step-4') : Args(2) {
    my ( $self, $c ) = @_;
    ...
    my $subscription = $c->model('Chargemonk::Subscription')->find( {id => $c->request->arguments->[1]} );
    $c->stash(
        {   template     => 'register/corporate/step-4.tt',
            subscription => $subscription,
        }
    );
