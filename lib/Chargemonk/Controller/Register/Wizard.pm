package Chargemonk::Controller::Register::Wizard;
...
Chargemonk::Controller::Register::Wizard - Catalyst Controller
...
sub step_1_confirmation : Path('/register/step-1-confirmation') : Args(2) : OnError(step_1) : Public {
    ...
    my $subscription = $c->model('Chargemonk::Subscription')->find( {id => $c->req->args->[1]} );
    my @features = $c->model('Chargemonk::LinkSubscriptionFeature')->search( {subscription_id => $subscription->id} )->all;
