package Chargemonk::Controller::Register;
use Moose;
use Module::Load;
...
Chargemonk::Controller::Register - Catalyst Controller
...
sub index : Path('/register') : Args(2) : Public {
    ...
    $c->forward( 'Chargemonk::Controller::Register::Wizard', 'step_1', [$c->req->args->[0], $c->req->args->[1]] );
...
sub activate_email : Path('/activate_email') : Args(0) : Public {
    ...
    my $registration_token =
        $c->model('Chargemonk::UserRegistrationToken')->search( {token => $c->req->param('token')} )->single;

    # TODO: Return message if no token

    if ($registration_token) {
        my $link_user_subscription =
            $c->model('Chargemonk::LinkUserSubscription')->find( {id => $registration_token->link_user_subscription_id} );
...
sub complete_registration : Local : Args(0) : Public {
    ...
    my $registration_token =
        $c->model('Chargemonk::UserRegistrationToken')->search( {token => $c->req->param('registration_token')} )->single;
    if ( $registration_token ) {        
