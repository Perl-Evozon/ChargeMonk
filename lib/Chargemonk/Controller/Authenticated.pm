package Chargemonk::Controller::Authenticated;
...
Chargemonk::Controller::Authenticated - base controller for all other Chargemonk controllers that need authentication

This module should be used instead of Catalyst::Controller as base for
all Chargemonk controllers that need authentication
...
sub auto : Private {
    ...
    my $theme = $c->model('Chargemonk::Theme')->search( {'active' => 1} )->single;
...
sub _validate : Private {
    ...
    foreach my $param ( keys %rules ) {
        ...
        while ( $count < scalar @{$rules{$param}} ) {
            my $validator_class = sprintf( "Chargemonk::ValidationRule::%s", $rules{$param}[$count++] );
