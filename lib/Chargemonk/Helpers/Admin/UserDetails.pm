package Chargemonk::Helpers::Admin::UserDetails;
...
Chargemonk::Helpers::Admin::UserDetails- Helpers for Chargemonk admin actions on 'User Details' page
...
sub send_set_user_psw_email {
    ...
    my $user = $c->model('Chargemonk::User')->find( {id => $id} );
    ...
    try {
        ...
        } until ( not $c->model('Chargemonk::UserPswSetToken')->find( {'token' => $token} ) );

        my $user_token = $c->model('Chargemonk::UserPswSetToken')->search( {uid => $id} )->slice( 0, 0 )->single;
        if ($user_token) {
            ...
            # create with token
            $c->model('Chargemonk::UserPswSetToken')->create(
                {   'uid'   => $id,
                    'token' => $token
                }
            );
