package Chargemonk::Helpers::Gateways::Braintree;
use Net::Braintree;
...
Chargemonk::Helpers::Gateways::Braintree- Helpers for Chargemonk payment gateway implemented with braintree
...
sub _add_user_and_pay {
    ...
    my $customer = $c->model('Chargemonk::BraintreeUser')->find( {user_id => $self->args->{user}->id} );
...
sub _add_customer_with_card {
    ...
    if ( $result && $result->is_success ) {
        ...
        try {
            $c->model('Chargemonk::BraintreeUser')->create(
