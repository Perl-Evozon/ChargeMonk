package Chargemonk::Helpers::Stats::Subscribers;
...
Chargemonk::Helpers::Stats::Subscribers
...
sub subscribers_stats {
    ...
    while ( $from le $to ) {
        ...
        my @new_subscribers = $c->model('Chargemonk::User')->search(
        ...
        my $active_subs_for_prev =
              $local_cache->{"$from"}
            ? $local_cache->{"$from"}
            : $c->model('Chargemonk::LinkUserSubscription')->search(
...
sub revenue_stats {
    ...
    while ( $from le $to ) {
        ...
        my @revenue_rs = $c->model('Chargemonk::LinkUserSubscription')->search(
...
sub subscription_stats {
    ...
    my @subscriptions = $c->model("Chargemonk::LinkUserSubscription")->search(
        {},
        {   join    => 'subscription',
            prefech => 'subscription',
...
sub revenue_prospect_stats {
    ...
    foreach my $month_to_add (@months_to_add) {
        ...
        my @subscription_prospect = $c->model('Chargemonk::LinkUserSubscription')->search({
            -and => [
