package Chargemonk::Helpers::Cron::Subscriptions;
...
use Chargemonk;
use Chargemonk::Helpers::Common::DateTime qw( timestamp );
use Chargemonk::Helpers::Gateways::Details qw(get_active_gateway get_gateway_credentials);
...
with 'Chargemonk::Helpers::Cron::Notifier';

=head1 NAME

Chargemonk::Helpers::Cron::Subscriptions
...
sub _build_handled_subscriptions {
    ...
    my @subs = $self->_app->model('Chargemonk::LinkUserSubscription')->search(
        {
    ...
    foreach my $element ( @{$self->handled_subscriptions()} ) {
        ...
        if ( $element->{process_message} eq 'expired' ) {
            ...
            if ( $prev_stage eq 'notify_expiring_subscription' ) {
                ...
                my @downgrade = $self->_app->model('Chargemonk::LinkUserSubscription')->search(
                    {
                        # only pick up the ones that are about to expire and the ones expired
...
sub _add_link_subscription {
    ...
    try {
        ...
        $new_link_user_subscription = $self->_app->model('Chargemonk::LinkUserSubscription')->create(
