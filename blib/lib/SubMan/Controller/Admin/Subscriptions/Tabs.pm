package SubMan::Controller::Admin::Subscriptions::Tabs;
...
SubMan::Controller::Admin::Subscriptions::Tabs - Catalyst Controller
...
sub features : Local : Args(1) {
    ...
    my $group         = $c->model('SubMan::SubscriptionGroup')->find( {id => $c->req->args->[0]} );
...
sub discount_codes : Local : Args(1) {
    ...
    my @campaigns = $c->model('SubMan::Campaign')->search($search_params);
    ...
    my @codes = $c->model('SubMan::Code')->search( {campaign_id => $campaign_id} );
    ...
    if ( defined $c->req->param('generate_codes_form') ) {
        ...
        @codes = $c->model('SubMan::Code')->search( {campaign_id => $campaign_id} );
    }
...
sub create_campaign {
    ...
    my $campaign;
    try {
        $campaign = $c->model('SubMan::Campaign')->create(
...
sub generate_codes {
    ...
    my $campaign = $c->model('SubMan::Campaign')->find($campaign_id);
