package SubMan::Controller::Admin::Subscriptions::edit_subscription;
use Moose;
...
SubMan::Controller::Admin::Subscriptions::edit_subscription - Catalyst Controller
...
sub features_tab : Local : Args(2) {
    ...
    my $subscription = $c->model('SubMan::Subscription')->find( {id => $c->req->args->[0]} );
    my $group = $c->model('SubMan::SubscriptionGroup')->find( {id => $c->req->args->[1]} );
    ...
    my @reload_subscription_features =
        $c->model('SubMan::LinkSubscriptionFeature')
    ...
    my @current_features_ids =
        ...
        ? @new_features_ids
        : map { $_->id }
        $c->model('SubMan::Feature')
        ->search( {subscription_id => $subscription->id}, {join => 'link_subscription_feature'} )->all();

    my @subscriptions = $c->model('SubMan::Subscription')->search(
        {   id                    => {'!=' => $subscription->id},
...
sub group_tab : Local : Args(2) {
    ...
    my @subscriptions = $c->model('SubMan::Subscription')->search( {subscription_group_id => $group->id} )->all();
...
sub upgrade_tab : Local : Args(2) {
    ...
    my @subscription_upgrade_to_ids =
        ...
        $c->model('SubMan::SubscriptionUpgradeTo')->search( {subscription_id => $subscription->id} )->all;
...
sub downgrade_tab : Local : Args(2) {
    ...
    my @subscription_downgrade_to_ids =
        ...
        $c->model('SubMan::SubscriptionDowngradeTo')->search( {subscription_id => $subscription->id} )->all;
