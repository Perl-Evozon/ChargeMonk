package SubMan::Controller::Admin::SubscriptionGroups;
...
SubMan::Controller::Admin::SubscriptionGroups - Catalyst Controller
...
sub add_group : Local {
	...
	if ( $c->req->param('name') ) {
		try {
			$c->model('SubMan::SubscriptionGroup')->create( {name => $c->req->param('name')} );
...
sub delete_group : Local {
	...
	if ( $c->req->param('id') ) {
		my $gr = $c->model('SubMan::SubscriptionGroup')->find( $c->req->param('id') );
		if ($gr) {
			try {
				$gr->delete();
