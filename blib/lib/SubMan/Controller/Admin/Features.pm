package SubMan::Controller::Admin::Features;
...
SubMan::Controller::Admin::Features - Catalyst Controller
...
sub features : Path : Args(0) {
	...
	my @features;
	@features = $c->model('SubMan::Feature')->all();
...
sub add_feature : Local : Args(0) {
	...
	my $feature;
	try {
		$feature = $c->model('SubMan::Feature')->create(
...
sub edit_feature : Local : Args(1) {
	...
	my $feature    = $c->model('SubMan::Feature')->find($feature_id);
