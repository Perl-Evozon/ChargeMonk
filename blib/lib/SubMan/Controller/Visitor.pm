package SubMan::Controller::Visitor;
use Moose;
...
SubMan::Controller::Visitor- Catalyst Controller
...
sub forgot_password : Global : Args(0) : Public {
    ...
    my $user = $c->model('SubMan::User')->find( {'email' => $c->req->param('email')} );
...
sub set_user_password : Global : Args(0) : Public {
    ...
    my $user_token = $c->model('SubMan::UserPswSetToken')->search( {'token' => $token} )->slice( 0, 0 )->single;
