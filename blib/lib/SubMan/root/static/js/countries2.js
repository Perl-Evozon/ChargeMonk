...
function populateCountry(defaultCountry, country_selector) {
  if ( postCountry != '' ) {
...
function populateState(country_selector, state_selector) {
  var selObj = document.getElementById(state_selector);
  ...
  //Find the abrev of the country
  var selected_countryCode = ' ';
  var countryLineArray = country.split('|');  // Split into lines
  for (var c_loop = 0; c_loop < countryLineArray.length; c_loop++) {
	c_lineArray = countryLineArray[c_loop].split(':');
	select_countryCode  = TrimString(c_lineArray[0]);
	selected_countryName  = TrimString(c_lineArray[1]);
	if (document.getElementById(country_selector).value == selected_countryName && selected_countryName != '' ) {
		selected_countryCode = select_countryCode;
	}
  }
  ...
  if (selected_countryCode == countryCode && countryCode != '' ) {
    // If it's an input element, change it to a select
      if ( selObj.type == 'text' ) {
        parentObj = document.getElementById(state_selector).parentNode;
        parentObj.removeChild(selObj);
        var inputSel = document.createElement("SELECT");
        inputSel.setAttribute("name",state_selector);
        inputSel.setAttribute("id",state_selector);
        inputSel.setAttribute("width", "85%");
        parentObj.appendChild(inputSel) ;
        selObj = document.getElementById(state_selector);
        selObj.options[0] = new Option('Select State','');
        selObj.selectedIndex = 0;
      }
      if ( stateCode != '' ) {
  ...
  // Create the Input Field
    ...
    inputEl.setAttribute("id", state_selector);
    inputEl.setAttribute("type", "text");
    inputEl.setAttribute("name", state_selector);
    inputEl.setAttribute("width", "80%");
    inputEl.setAttribute("value", postState);
    parentObj.appendChild(inputEl) ;
  }
}

function initCountry(country, country_select, state_select) {
  populateCountry(country, country_select);
  populateState(country_select, state_select);
}