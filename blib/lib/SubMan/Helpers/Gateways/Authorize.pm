package SubMan::Helpers::Gateways::Authorize;
...
=head2
    Arguments necessary to create the object:
        args => {
            c                          => $c, #required
            gateway_credentials        => $gateway_credentials, #required ( SubMan::Helpers::Gateways::Details::get_gateway_credentials( $c, $gateway ) )
...
sub _add_user_and_pay {
    ...
    my $customer = $c->model('SubMan::AuthorizeUser')->find( {user_id => $self->args->{user}->id} );
...
sub _add_customer_with_card {
    ...
    if ( $new_customer->{messages}->{resultCode} eq 'Ok' ) {
        ...
        try {
            $c->model('SubMan::AuthorizeUser')->create(
