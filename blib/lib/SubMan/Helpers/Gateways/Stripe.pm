package SubMan::Helpers::Gateways::Stripe;
use Business::Stripe;
...
sub _add_user_and_pay {
    ...
    my $customer = $self->args->{c}->model('SubMan::StripeUser')->find( {user_id => $self->args->{user}->id} );
...
sub _pay {
    ...
    my $customer = $c->model('SubMan::StripeUser')->find( {user_id => $self->args->{user}->id} );
...
sub _add_customer_with_card {
    ...
    if ($customer_id) {
        ...
        try {
            $c->model('SubMan::StripeUser')->create(
