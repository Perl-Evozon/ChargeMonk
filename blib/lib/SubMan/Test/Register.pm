...
#
    ...
    foreach my $type (@subscription_type) {
        my $subscription =
            ( $type eq 'trial' )
            ? $self->mech->_app->model('SubMan::Subscription')->create($test_trial_subscription_data)
            : $self->mech->_app->model('SubMan::Subscription')->create($test_subscription_data);

#? $self->mech->_app->model('SubMan::Subscription')->search( { name => $test_trial_subscription_data->{name} } )->first()
#: $self->mech->_app->model('SubMan::Subscription')->search( { name => $test_subscription_data->{name} } )->first();
...
=cut
        ...
        my $required_data = $self->mech->_app->model('SubMan::Registration')->find( {id => 1} );
        my @cols = (
        ...
        my $existing_user          = $self->mech->_app->model('SubMan::User')->create($existing_user_data);
        ...
        my $residual_user =
            $self->mech->_app->model('SubMan::User')->search( {email => $required_user_cols{email}} )->first();
...
sub register_with_gateway {
    ...
    my $subscription = $self->mech->_app->model('SubMan::Subscription')->search( {id => $subscription_id} )->first();
