...
my $last_run_obj = $c->model("SubMan::Config")->find('last_migration_run');
my $first_time   = 0;
if ( !$last_run_obj ) {    # first run
    $first_time = 1;

    #add the entry in the config table
    $last_run_obj = $c->model("SubMan::Config")->create(
...
my $dbh = $c->model('SubMan')->storage->dbh;
...
subman_create_admin.pl - Apply SQL migrations for the project
