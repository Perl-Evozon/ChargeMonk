...
(function ($, undefined) {
    ...
    function local(options) {
        ...
        return function (query) {
            ...
            process = function(chargemonk, collection) {
                var group, attr;
                chargemonk = chargemonk[0];
                if (chargemonk.children) {
                    group = {};
                    for (attr in chargemonk) {
                        if (chargemonk.hasOwnProperty(attr)) group[attr]=chargemonk[attr];
                    }
                    group.children=[];
                    $(chargemonk.children).each2(function(i, childChargemonk) { process(childChargemonk, group.children); });
                    if (group.children.length) {
                        collection.push(group);
                    }
                } else {
                    if (query.matcher(t, text(chargemonk))) {
                        collection.push(chargemonk);
                    }
                }
            };

            $(data).each2(function(i, chargemonk) { process(chargemonk, filtered.results); });
